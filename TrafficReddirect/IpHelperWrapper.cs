...
using System.Text;
using static TrafficReddirect.TCPUDPConnection;

namespace TrafficReddirect {
	public static class IpHelperWrapper {

		public static List<TCPUDPConnection> GetTcpConnections(string processName) {
			int AF_INET = 2;    // IP_v4
			int buffSize = 20000;
			...
			if (res != 0) //If there is no enouth memory to execute function
			{
			...
			nOffset += 4;
			var localHostname = Utils.GetLocalHostName();
			var listOfTcp = new List<TCPUDPConnection>();

			for (int i = 0; i < NumEntries; i++) {
				var row = new TCPUDPConnection(localHostname);
				// state
				int st = Convert.ToInt32(buffer[nOffset]);
				// state  by ID
				row.iState = st;
				nOffset += 4;
				row.Protocola = Protocol.TCP;
